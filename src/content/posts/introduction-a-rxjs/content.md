...
A la fin de cet article, si j'ai pas trop mal fait mon boulot, vous devriez avoir les bases pour comprendre le fonctionnement de RxJS, et ce que ça apporte. Cependant, il faudra ensuite pas mal de pratique pour en tirer le meilleur.
...
* De données qui viennent d'actions de l'utilisateur ?
    Ca peut être déclenché au scroll, au click ou à la soumission d'un formulaire.
    ```js
...
* De données qui viennent d'un tableau ?
    La donnée n'est pas asychrone. Ca ne nous empêche pas d'intéragir avec elle.
    ```js
...
Cela fait un sacré paquet de possibilités déjà. Pourtant dans chacun des exemples ci-dessus le but est d'afficher un message via la méthode `displayMessage`.
...
C'est une façon de penser différente par rapport à ce à quoi nous sommes habitués. Ca va donc demander de l'entraînement, et il y a de fortes chances qu'à la fin de ce premier tutoriel, ce soit encore flou pour vous. Mais je vais essayer de semer des petites graines qui finiront par germer. <small>Parce que c'est le printemps et que la vie, c'est cool.</small>

A mon sens, la plus grande différence réside dans le rôle de la source de données. Dans notre exemple, elle ne devra plus *afficher un message* mais *prévenir qu'il y a un message*. Ce ne sera plus qu'une banque de données. Quiconque pourra venir s'y enregistrer et réagir à l'arrivée d'une nouvelle donnée, mais elle n'aura pas d'autre responsabilité que celle de récupérer les données.

Ainsi, la valeur ajouté de notre code sera uniquement dans la façon dont on réagit aux données, et non plus dans la provenance des données qui sont censées être des détails d'implémentation. Par exemple, lorsqu'on veut afficher un message, ce qui est important, c'est comment on l'affiche. Mais qu'il vienne d'une API REST ou d'une WebSocket, ce n'est pas censé impacter l'affichage de celui-ci.

### Coder réactif

Comment est-ce que RxJS s'insère dans ce mode de pensée ? Il va falloir qu'il réponde à deux question&nbsp;:
...
* Event listener
  ...
  const source = Observable.create((observer) => {
    ...
    element.addEventListener('click', (event) => {
      // A chaque nouveau clic, on ajoute l'objet
...
Cependant, le problème c'est que, si on utilise directement `subscribe` après la première source, on revient à la case départ puisque ce sera dans le `subscribe` que tout sera régi. Ce sera un poil plus tard et donc un poil moins problématique, mais il y aura à nouveau un endroit dans le code qui sera responsable de faire les choses plutôt que prévenir les autres d'un nouvel état.

Par exemple, dans le cadre de l'affichage d'un message. Admettons que le but est d'afficher tous les messages reçus et non le dernier. Cela veut dire qu'il nous est plus utile d'avoir une source de données qui contient l'ensemble des messages à chaque `next` plutôt que de devoir les agglomérer nous même dans la fonction d'affichage. On va donc devoir faire une opération sur les sources initiales pour en faire une nouvelle qui contient tous les messages à chaque `next` à chaque fois.
...
const listSource = Observable.create((observer) => {
  // Stockons la liste des messages reçus
  // C'est cette liste qui devra être mise
  // à disposition par l'observable
  let messages = []

  // Le but de cette source est de réagir
  // aux données d'une autre source. On doit
  // donc s'inscrire à la première
  source.subscribe(
    (message) => {
  ...
  // S'il y a besoin de plus de sources,
  // on peut ajouter d'autres subscribes ici
})
```

Et voilà&nbsp;! Vous avez une nouvelle source de donnée qui contient la liste des messages complète à chaque fois qu'un nouveau message arrive. Elle vous sera infiniement plus utile lorsque vous aurez besoin d'afficher la liste complète des messages dans l'application que la source qui envoie les messages un par un.

Le hic, c'est que ça fait beaucoup de code et ce n'est pas forcément très lisible. Heureusement, le jour où vous lirez du code écrit en Rx, vous ne verrez jamais rien de tel. En fait, je vous montre ça pour bien vous montrer que lorsqu'on utilise les *opérateurs* en Rx, cela ne fait finalement rien de magique. Les opérateurs se contentent de créer un nouvel *Observable* (`listSource`) qui récupère ses données depuis le précédent (`source`).
...
C'est la fin de ce tutoriel sur RxJS. A vous de me dire maintenant si j'ai bien fait mon boulot ou non. :)

Dans tous les cas, s'il y n'avait que quelques points à retenir, ce serait ceux-là :

* Le but de la programmation réactive est d'éviter de donnée trop d'importance à la source de données
