...
class InterruptAnimation extends React.Component {
  ...
  componentDidUpdate () {
    if (this.props.withoutLayoutThrashing) {
      this.initialScale = Object.keys(this.node)
        ...
        .reduce((initialScale, scale, index) => ({
          ...initialScale,
          [index]: scale
        }), {})
    } else {
  ...
  getScale (progress, index) {
    if (!this.initialScale[index]) {
      if (this.props.checkStart) {
        ...
        this.initialScale[index] = this.state.opened ? CLOSED_SCALE : EXPANDED_SCALE
      }
    }

    if (this.state.opened) {
      return this.initialScale[index] + (EXPANDED_SCALE - this.initialScale[index]) * progress
    } else {
      return this.initialScale[index] - (this.initialScale[index] - CLOSED_SCALE) * progress
    }
  }
  ...
  render () {
    return <div style={{maxHeight: 180, overflowY: 'auto'}}>
