		...
		return (
			<SpyProvider>
				<SpyTarget name="container">
					{({ setSpyTarget }) => (
						<div style={{ position: 'relative', margin: '1.5em 0' }} ref={setSpyTarget}>
							{this.state.tooltip && (
								<Tooltip
									{...this.state.tooltip}
							...
							<SpySubscriber>
								{({ container }) => {
									const { data, viewBox } = this.computeState({
										...this.props,
										width: container ? container.getBoundingClientRect().width : DEFAULT_WIDTH
									});
									const lines = this.getLines(data);

									return <svg viewBox={`0 0 ${viewBox.width} ${viewBox.height}`}>{lines}</svg>;
								}}
							</SpySubscriber>
						</div>
					)}
