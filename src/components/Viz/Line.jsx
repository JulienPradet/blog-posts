...
const getInitialContext = (width, legend, length, color) => {
	...
	const separationWidth = Math.max(lineWidth / (length + 1), ELEMENT_SIZE * 1.25);
...
Line.getData = (props, parentContext) => {
	...
	const isComplete = Children.toArray(props.children).find((child) => child.type === IsComplete);

	const ElementsComponents = Components.map((Component, index) => {
		return ({ onMouseEnter, onMouseLeave }) => (
			<Component key={index} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} />
		);
	});
	...
	const isComplete = (
		<g transform={`translate(${width}, ${startLineOffset.top})`}>
			{props.isComplete ? (
				...
				<path d="M -3 -2 L 0 0 L -3 2" stroke={props.color} fill={props.color} />
			)}
		</g>
	);
